...
class Tracker {
	...
	public function __construct() {
		global $wpdb;
		$this->table = $wpdb->prefix . 'rs_order_shield_incomplete_orders';

		add_action( 'wp', array( $this, 'maybe_track_cart_or_checkout' ) );
		add_action( 'wp_enqueue_scripts', array( $this, 'get_checkout_info_script' ) );
		add_action( 'wp_ajax_rs_capture_checkout_data', array( $this, 'rs_capture_checkout_data' ) );
		add_action( 'wp_ajax_nopriv_rs_capture_checkout_data', array( $this, 'rs_capture_checkout_data' ) );

		add_action( 'woocommerce_checkout_order_processed', array( $this, 'cleanup_on_order_success' ), 10, 3 );
		add_action( 'woocommerce_payment_complete', array( $this, 'cleanup_on_order_success' ), 10, 1 );
		add_action( 'woocommerce_thankyou', array( $this, 'cleanup_on_order_success' ), 10, 1 );
	}
