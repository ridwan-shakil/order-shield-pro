...
final class Plugin {
    ...
    public function i($hook) {
        ...
        if ($hook === 'toplevel_page_rs-order-blocker' || in_array($p, ['rs-order-blocker-settings', 'rs-failed-orders', 'rs-blocked-users', 'rs-fraud-analytics'])) {
            wp_enqueue_style('ob-ui', $files . 'admin-license.css', [], RS_ORDER_SHIELD_PRO_VERSION);
            wp_enqueue_style('ob-alert', $files . 'admin-license-alert.css', [], RS_ORDER_SHIELD_PRO_VERSION);
        }
    }
...
<?php
    ...
    private function q() {
        ...
        $res = wp_remote_get('https://dev-mdridwan.pantheonsite.io/wp-json/lmfwc/v2/licenses/activate/' . rawurlencode($k), [
            'headers' => [
    ...
    private function f() {
        ...
        $res = wp_remote_get('https://dev-mdridwan.pantheonsite.io/wp-json/lmfwc/v2/licenses/validate/' . rawurlencode($k), [
            'headers' => [
                'Authorization' => 'Basic ' . base64_encode('ck_f052ac22415d8bee26c3ac563fdb32b147df7c47:cs_6975d1fe585719688a62303740e55967b98d4ca1')
            ],
            'timeout' => 30
