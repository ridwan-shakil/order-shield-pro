...
class Assets {
    ...
    public function frontend_styles() {
        ...
        wp_enqueue_style('rs-blocker-style', plugin_dir_url(__DIR__) . 'assets/css/frontend.css', [], RS_ORDER_SHIELD_PRO_VERSION);

        wp_enqueue_script('rs-order-blocker-script', plugin_dir_url(__DIR__) . 'assets/js/frontend.js', ['jquery'], RS_ORDER_SHIELD_PRO_VERSION, true);
    }

    public function admin_styles($hook) {
        // Admin Faild orders page css 
        if (!isset($_GET['page']) || $_GET['page'] == 'rs-failed-orders') {
            wp_enqueue_style('rs-faild-orders-page-css', plugin_dir_url(__DIR__) . 'assets/css/admin-faild-orders-page.css', [], RS_ORDER_SHIELD_PRO_VERSION);
        }
        // Admin settings page css 
        if (strpos($hook, 'rs-order-blocker') === false) return;
        wp_enqueue_style('rs-blocker-admin', plugin_dir_url(__DIR__) . 'assets/css/admin-settings.css', [], RS_ORDER_SHIELD_PRO_VERSION);
    }
    ...
    public function fraud_analetics_page_script($hook) {
        ...
        // custom assets
        wp_enqueue_style('rs-fraud-analetics-css', plugin_dir_url(__DIR__) . 'assets/css/admin-fraud-analetics.css', [], RS_ORDER_SHIELD_PRO_VERSION);

        wp_register_script('rs-fraud-analetics-js', plugin_dir_url(__DIR__) . 'assets/js/admin-fraud-analetics-page.js', ['jquery'], RS_ORDER_SHIELD_PRO_VERSION, true);


        // map assets cdn link
        wp_enqueue_style('leaflet-css', 'https://unpkg.com/leaflet@1.9.4/dist/leaflet.css', [], RS_ORDER_SHIELD_PRO_VERSION);
        wp_enqueue_script('leaflet-js', 'https://unpkg.com/leaflet@1.9.4/dist/leaflet.js', [], RS_ORDER_SHIELD_PRO_VERSION, true);
