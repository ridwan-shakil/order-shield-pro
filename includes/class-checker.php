...
class Checker {
	...
	public function init() {
		...
		add_action( 'woocommerce_checkout_process', array( $this, 'check_block' ) );

		// Save meta as soon as order object exists (BEFORE blocking check)
		add_action( 'woocommerce_checkout_create_order', array( $this, 'save_meta' ), 5, 1 ); // classic
		add_action( 'woocommerce_store_api_checkout_update_order_from_request', array( $this, 'save_meta' ), 5, 1 ); // blocks (first arg is WC_Order)
		...
		// CartFlows AJAX
		add_action( 'wp_ajax_wc_cartflows_submit_checkout', array( $this, 'check_block_ajax' ) );
		add_action( 'wp_ajax_nopriv_wc_cartflows_submit_checkout', array( $this, 'check_block_ajax' ) );

		// Store API: run the blocker after the order is created from request (but before finalization)
		add_action( 'woocommerce_store_api_checkout_update_order_from_request', array( $this, 'block_checkout_after_order' ), 10, 2 );
	}
	...
	private function get_settings() {
		return get_option( 'rs_order_blocker_settings', array() );
	}
	...
	public function check_block_ajax() {
		...
		if ( $this->is_blocked( $phone, $email, $ip, $device_id, $settings ) ) {
			wp_send_json_error( array( 'messages' => $this->get_popup_html( $settings ) ) );
		}
		...
		if ( ! empty( $settings['enable_number_validation'] ) ) {
			...
			if ( $phone_raw !== '' && ! $this->is_valid_phone_bd( $phone_raw ) ) {
				...
				throw new \WC_REST_Exception(
					'rs_invalid_phone',
					$msg ?: __( 'দুঃখিত, আপনি একটি ভুল নাম্বার লিখেছেন। দয়া করে সঠিক বাংলাদেশি নাম্বার লিখুন:', 'ninja-fake-order-blocker' ),
					400
				);
			}
		}
