...
class FailedOrdersTable extends \WP_List_Table {
	...
	public function prepare_items() {
		...
		// Sortable columns
		$sortable_columns = array( 'id', 'status', 'total', 'created_at' );
		...
		// search , status filter and product filters are Not working
		...
		if ( in_array( $status_filter, array( 'cart', 'checkout', 'payment_failed' ), true ) ) {
		...
		if ( ! empty( $product_filter ) ) {
			...
			$all_data             = array_filter(
				$all_data,
				function ( $item ) use ( $product_filter_lower ) {
	...
	// To show the "Others info" column data in human readable format (it was serialized)
