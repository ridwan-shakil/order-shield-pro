...
class FailedOrdersPage {
	...
	public static function render() {
		...
		// Use GET instead of POST so filters & search work
		echo '<form method="get">';

		// Keep page slug so WP knows which admin page this is
		echo '<input type="hidden" name="page" value="' . esc_attr( $_GET['page'] ?? '' ) . '" />';

		// Preserve orderby/order when filtering/searching
		...
		$export_url = wp_nonce_url(
			add_query_arg( array( 'rs_export_csv' => 1 ), admin_url( 'admin.php?page=rs-failed-orders' ) ),
			'rs_export_csv'
		);
	...
	private static function export_failed_orders_to_csv() {
		global $wpdb;
		$table = $wpdb->prefix . 'rs_order_shield_incomplete_orders';

		$where  = array();
		$params = array();

		// Filters
		if ( ! empty( $_GET['status_filter'] ) && in_array( $_GET['status_filter'], array( 'cart', 'checkout', 'payment_failed' ), true ) ) {
			$where[]  = 'status = %s';
		...
		fputcsv( $output, array( 'ID', 'Status', 'Name', 'Mobile', 'Email', 'Product Info', 'Cart Total', 'Last Visited' ) );
