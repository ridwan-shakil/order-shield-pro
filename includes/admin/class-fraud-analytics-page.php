...
class FraudAnalyticsPage {
	...
	public static function render() {
		$tabs = array(
			...
			'customer_journey' => __( '🏁 Customer Journey', 'wcorder-blocker' ),
		...
		switch ( $current ) {
			...
			case 'customer_journey':
				self::render_customer_journey_section();
				break;
			case 'heatmap':
	...
	public static function render_heatmap_section() {
		...
		if ( $is_hpos ) {
			$orders = $wpdb->get_results(
				" SELECT o.id, a.state, s.status, s.total_sales AS total, a.email, a.phone, s.date_created_gmt
        ...
        ",
			...
			$orders = $wpdb->get_results(
				" SELECT p.ID, pm1.meta_value AS state, pm2.meta_value AS total,
		...
		foreach ( $orders as $order ) {
			...
			if ( in_array( $status, array( 'wc-cancelled', 'wc-failed', 'wc-refunded' ) ) ) {
				++$district_stats[ $district ]['failed_orders'];
				++$total_failed;
			}

			if ( $email ) {
				$district_stats[ $district ]['emails'][] = $email;
			}
			if ( $phone ) {
				$district_stats[ $district ]['phones'][] = $phone;
			}
	...
	public static function get_refusal_chart_data( $from, $to ) {
		...
		if ( $is_hpos ) {
			$total_rows = $wpdb->get_results(
				$wpdb->prepare(
					" SELECT DATE_FORMAT(date_created_gmt,'%Y-%m') month, COUNT(*) total_cnt
            FROM {$wp_}wc_orders
        ...
        ",
			...
			$total_rows = $wpdb->get_results(
				$wpdb->prepare(
					" SELECT DATE_FORMAT(post_date_gmt,'%Y-%m') month, COUNT(*) total_cnt
            FROM {$wp_}posts
		...
		if ( $is_hpos ) {
			$refusal_rows = $wpdb->get_results(
				$wpdb->prepare(
					" SELECT DATE_FORMAT(date_created_gmt,'%Y-%m') month, COUNT(*) refuse_cnt
            FROM {$wp_}wc_orders
        ...
        ",
			...
			$refusal_rows = $wpdb->get_results(
				$wpdb->prepare(
					"  SELECT DATE_FORMAT(post_date_gmt,'%Y-%m') month, COUNT(*) refuse_cnt
            FROM {$wp_}posts
            WHERE post_type = 'shop_order'
	...
	// =================== Customer Journey ==================
	private static function render_customer_journey_section() {
		require_once plugin_dir_path( __FILE__ ) . 'class-admin-workflow-page.php';

		render_fop_workflow_tree_page();
	}
}
